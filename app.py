...
<style>
    ...
    .question-title { color: #1A5276; font-size: 16px; font-weight: bold; border-bottom: 1px solid #D4E6F1; padding-bottom: 5px; margin-top: 20px; margin-bottom: 15px; }
...
career_database = [
    ...
    {'interests': ['Tecnologia'], 'education': ['Ensino Médio', 'Ensino Superior', 'Pós-Graduação'], 'career': {'title': 'Desenvolvedor de Software', 'description': 'Trilha FullStack', 'reason': 'Seu perfil analítico combina com engenharia.', 'image': 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?q=80&w=400'}},

    {'interests': ['Artes & Design'], 'education': ['Ensino Médio', 'Ensino Superior'], 'career': {'title': 'Designer UX/UI', 'description': 'Interfaces Digitais', 'reason': 'Sua criatividade focada no usuário.', 'image': 'https://images.unsplash.com/photo-1581291518857-4e27b48ff24e?q=80&w=400'}}
...
def get_career_recommendation(selected_interests, selected_education):
    ...
    return matches if matches else [{'title': 'Empreendedor Digital', 'description': 'Gestão de Negócios', 'reason': 'Perfil inovador.', 'image': 'https://images.unsplash.com/photo-1542744094-3a31f2f31d4d?q=80&w=400'}]
...
def show_results_page():
    ...
    for career in st.session_state.recommended_careers:
        ...
        st.markdown(f'<div class="rec-card"><img src="{career["image"]}" class="rec-card-img"><div><div class="rec-card-title">{career["title"]}</div><div>{career["description"]}</div></div></div>', unsafe_allow_html=True)
